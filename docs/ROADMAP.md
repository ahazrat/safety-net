...
| Station directory (Chicago CPD/CFD map pins) | Shipped: hand-seeded pins + Map toggles (not a full directory UI) |
...
3. **Stations.** Chicago CPD districts/HQ + CFD houses seeded as map pins (Map toggles). Hand-verified corrections; no national bulk import.
...
2. **One real roster:** one Chicago neighborhood, not a national import. Steps are in [`docs/ops-chicago.md`](ops-chicago.md): 5–10 providers with admin-granted badges, five real listings, then walk create → map → accept → DM → start → done.
